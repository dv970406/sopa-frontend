...
export default function SeePostDetail({
  pageTitle,
  seePost,
  fetchMore,
  comments,
}: ISeePostDetailComponent) {
  // 토큰이 있다면 로그인 상태이고 없다면 비로그인 상태이므로 이를 분기처리하여 CreateComment를 제공할 것인지 결정한다.
  const token = useRecoilValue(tokenState);

  return (
    <>
      <div className="flex items-center justify-between w-full p-3 border-b-2 border-b-sopa-default">
        <h1 className="text-4xl font-bold ">{seePost?.title || pageTitle}</h1>
        {seePost?.isMine ? (
          <MenuBtn postId={seePost?.id} />
        ) : (
          <AuthorGithubLink user={seePost?.user} />
        )}
      </div>
      {seePost?.id ? (
        <div className="w-full py-3 border-b-2 border-b-sopa-default">
          ...
          <p className="break-words whitespace-pre-wrap py-7 text-md lg:text-lg">
            {seePost?.description}
          </p>
          <div
            className={`
          ...
          >
            {seePost?.openChatLink ? (
              <OpenChatLink openChatLink={seePost?.openChatLink} />
            ) : null}
            <MetaData
              postId={seePost?.id}
